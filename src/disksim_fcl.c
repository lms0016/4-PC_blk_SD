...
// record_req( ioreq_event *tmp ) { // fox
// reset_record() { // fox
...
// printf("dev:%d flags:%d list_index:%d\n", devno, flags, list_index); // foxtodelete
	ioreq_event *child = NULL;
...
// printf("fcl_make_seq"); // foxtodelete
...
// printf("fcl_make_normal"); // foxtodelete
	struct lru_node *ln = NULL;
	int hit = 0;
	int goHDD = 0; 
...
// printf("_fcl_make_stage "); // foxtodelete
...
// printf("_fcl_make_destage "); // foxtodelete
...
void fcl_make_request ( ioreq_event *parent, int blkno ) {
	switch ( parent->tempint1 ) {
		...
		*/
			...
			if ( sd_is_seq_io(blkno)) 
				fcl_make_seq_req(parent, blkno); // seq req
			else 
...
void fcl_get_next_request ( int op_type ) {
	...
	if ( op_type == FCL_OPERATION_NORMAL ) {
		...
		sd_seq_detection ( req->blkno, req->bcount); // foxtodo

		if ( !sd_is_seq_io ( req->blkno ) ) // foxtodo
			fcl_update_workload_tracker ( req ); // foxtodo
...
//printf("blkno:%d\t bcount:%d\t flag:%d\t devno:%d\n", parent->blkno, parent->bcount, parent->flags, parent->devno); // foxtodelete
